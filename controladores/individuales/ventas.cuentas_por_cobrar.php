...
class ventasCxc extends alkesGlobal
{
    function inializarFormulario()
    {
        ...
        if (validaPermisoEditarModulo($modulo, $submodulo, $subsubmodulo)) {
            $this->response->append("botonera-contenedor", "innerHTML", "
                <button class='btn btn-primary btn-sm' type='button' id='btnborrador' name='btnborrador' onclick='JaxonventasCxc.validar(jaxon.getFormValues(\"formulario{$rand}\"), \"borrador\");'>
                    <i class='bi bi-floppy'></i> Borrador
                </button>
            ");
            $this->response->append("botonera-contenedor", "innerHTML", "
                <button class='btn btn-success btn-sm' type='button' id='btnprocesar' name='btnprocesar' onclick='JaxonventasCxc.validar(jaxon.getFormValues(\"formulario{$rand}\"), \"procesar\");'>
                    <i class='bi bi-check2-circle'></i> Procesar
                </button>
            ");
            $this->response->append("botonera-contenedor", "innerHTML", "
                <button class='btn btn-danger btn-sm' type='button' id='btncancelar' name='btncancelar' onclick='JaxonventasCxc.modalConfirmacionCancelar();'>
        ...
        $this->response->append("botonera-contenedor", "innerHTML", "
            <button class='btn btn-secondary btn-sm' type='button' id='btnimprimir' name='btnimprimir' onclick='JaxonventasCxc.imprimir();'>
        ...
        if ($_GET['id'] == 0) {
            ...
            if (!validarEmpresaPorRegistro("cuentas_cargos", $_GET['id'])) {
                ...
                $this->response->script('
                    ...
                    $("#idvendedor").val("' . $cuenta_cargo['idvendedor'] . '").trigger("change.select2");
                    $("#idc_moneda").val("' . $cuenta_cargo['idc_moneda'] . '").trigger("change.select2");

                    document.getElementById("idsocio").onchange = function() {
                        JaxonventasCxc.cargarSocio({ seleccion: this.value });
                    };
                    document.getElementById("idsubcuenta").onchange = function() {
                        JaxonventasCxc.cargarSubcuenta({ seleccion: this.value }, document.getElementById(\'idsocio\').value);
                    };
                ');
                ...
                if ($cuenta_cargo['estado'] != 'Borrador') {
                    ...
                    $this->deshabilitaSelect('idvendedor');
    ...
    function modalSeleccionarSocio()
    {
        ...
        $this->modalSeleccionServerSide('ventas', 'clientes', '', 0, 'Activos', 'Modal', 'JaxonventasCxc.cargarSocio', false, '', 'Selecciona Un Cliente');
        return $this->response;
    }
    ...
    function cargarSocio($form)
    {
        ...
        $this->response->script('
            ...
            document.getElementById("idsocio").onchange = function() {
                JaxonventasCxc.cargarSocio(this.value);
            };
        ');
    ...
    function modalSeleccionarSubcuenta($idsocio)
    {
        ...
        $this->modalSeleccion(
            ...
            $columnas,
            $data,
            "JaxonventasCxc.cargarSubcuenta",
            true,
            false,
            ", $idsocio"
        );
    ...
    function cargarSubcuenta($form, $idsocioPrincipal)
    {
        ...
        $this->response->script('
            ...
            // Restaurar el evento onchange original para los selects que tienen onchange y necesiten un change (select2)
            document.getElementById("idsubcuenta").onchange = function() {
                JaxonventasCxc.cargarSubcuenta(this.value);
            };
        ');
        return $this->response;
    }
    ...
    function modalAgregarAbono($form)
    {
        ...
        $campos = [
            ...
            [
                ...
                'label' => 'Concepto',
                'type' => 'select',
                'options' => getConceptosCuentas('Abono', 'CXC'),
        ...
        $funcionCallBack = 'JaxonventasCxc.validarAbono'; // Nombre de la función JavaScript
    ...
    function registrarAbono($formModal)
    {
        ...
        try {
            ...
            $data = [
                ...
                'folio' => getConsecutivo('ventas_cxc_abono'),
    ...
    function modalPreCancelarAbono($idabono)
    {
        ...
        $this->alertaConfirmacion(
            "¿CANCELAR?",
            "Se cancelará el abono seleccionado, alterando el saldo del cliente. ¿Desea continuar?",
            "warning",
            "JaxonventasCxc.cancelarAbono($idabono);"
        );
    ...
    function modalPreGuardar($form, $accion)
    {
        ...
        if ($accion == 'procesar') {
            registroLog('modalPreGuardar', 'Mostrando modal alerta de confirmación para procesar la cxc', []);
            ...
            $this->alertaConfirmacion(
                "¿PROCESAR?",
                "Se procesará la cuenta por cobrar, alterando el saldo del cliente seleccionado. ¿Desea continuar?",
                "warning",
                "JaxonventasCxc.guardar(JSON.parse(\"$formJsonEscapado\"), \"$accion\");"
            );
        }
        return $this->response;
    }
    ...
    function guardar($form, $accion)
    {
        ...
        registroLog('guardar', 'Guardando cuenta por cobrar', ['accion' => $accion]);
        ...
        try {
            ...
            if ($idCxc == 0) {
                $folio = getConsecutivo('ventas_cxc_cargo');
                registroLog('guardar', 'Nueva cxc, folio generado', ['folio' => $folio]);
            } else {
                // Recuperar el folio actual desde la base de datos para mantenerlo
                $folio = $database->get('cuentas_cargos', 'folio', ['id' => $idCxc]);
                registroLog('guardar', 'CXC existente, folio recuperado', ['folio' => $folio]);
            }
            ...
            if ($accion == 'borrador') {
                if ($idCxc == 0) {
                    ...
                    $insert_id = $database->id();
                    registroLog('guardar', 'Nuevo borrador insertado', ['idcxc' => $insert_id]);
                    $this->alerta(
                        "Éxito",
                        "Cuenta por cobrar guardada como borrador correctamente.",
                        "success",
                        null,
                        true,
                        false,
                    ...
                    $database->update('cuentas_cargos', $data, ['id' => $idCxc]);
                    registroLog('guardar', 'Borrador actualizado', ['idcxc' => $idCxc]);
                    $this->alerta(
                        "Éxito",
                        "Cuenta por cobrar actualizada como borrador correctamente.",
                        "success",
                        null,
                        true,
                        false,
                ...
                if ($idCxc == 0) {
                    ...
                    $database->insert('cuentas_cargos', $data);
                    $insert_id = $database->id();
                    registroLog('guardar', 'Nueva CXC procesado', ['idcxc' => $insert_id]);
                    $this->alerta(
                        "Éxito",
                        "Cuenta por cobrar procesada correctamente.",
                        "success",
                        null,
                        true,
                        false,
                    ...
                    $database->update('cuentas_cargos', $data, ['id' => $idCxc]);
                    registroLog('guardar', 'Cuenta por cobrar actualizada y procesada', ['idcxc' => $idCxc]);
                    $this->alerta(
                        "Éxito",
                        "Cuenta por cobrar actualizada y procesada correctamente.",
                        "success",
                        null,
                        true,
                        false,
            ...
            $database->pdo->commit();
            registroLog('guardar', 'Transacción confirmada', ['idcxc' => $idCxc]);
        } catch (PDOException $e) {
    ...
    function modalConfirmacionCancelar()
    {
        ...
        registroLog('modalConfirmacionCancelar', 'Validando si se puede cancelar la cuenta por cobrar', ['idcargo' => $_GET['id']]);
        ...
        if (!$cargo) {
            ...
            $this->alerta("Error", "La cuenta por cobrar no fue encontrada. Por favor, contacta al administrador del sistema.", "error");
            return $this->response;
        }

        // Verificar si está relacionado a otro módulo
        if (!empty($cargo['idreferencia']) && !empty($cargo['referencia'])) {
            registroLog('modalConfirmacionCancelar', 'La cuenta por cobrar está vinculada a otro módulo. Cancelación no permitida desde aquí.', ['idcargo' => $_GET['id']]);
            $this->alerta(
                "Advertencia",
                "Esta cuenta por cobrar está vinculada a otro módulo y no puede ser cancelada desde aquí. Por favor, cancélala en su módulo correspondiente.",
                "warning"
            );
            return $this->response;
        }
        ...
        $this->alertaConfirmacion(
            "¿CANCELAR?",
            "Se cancelará esta cuenta por cobrar, alterando el saldo del cliente. ¿Desea continuar?",
            "warning",
            "JaxonventasCxc.cancelarCargo();"
        );
    ...
    function cancelarCargo()
    {
        ...
        registroLog('cancelarCargo', 'Intentando cancelar cuenta por cobrar', ['idcargo' => $_GET['id']]);
        ...
        try {
            ...
            if (!$cargo) {
                registroLog('cancelarCargo', 'Cuenta por cobrar no encontrada', ['idcargo' => $_GET['id']]);
                $this->alerta("Error", "La cuenta por cobrar no fue encontrada, por favor contacta al administrador del sistema.", "error");
                return $this->response;
            }
            ...
            if ($cargo['estado'] == 'Cancelada') {
                registroLog('cancelarCargo', 'La cuenta por cobrar ya está cancelada', ['idcargo' => $_GET['id']]);
                $this->alerta("Advertencia", "Esta cuenta por cobrar ya está cancelada.", "warning");
                return $this->response;
            }
            ...
            if ($abonosConfirmados > 0) {
                registroLog('cancelarCargo', 'La cuenta por cobrar tiene abonos confirmados', ['idcargo' => $_GET['id']]);
                $this->alerta(
                    "Error",
                    "No se puede cancelar esta cuenta por cobrar porque tiene abonos confirmados. Primero cancela los abonos asociados.",
                    "error"
                );
                return $this->response;
            }
            ...
            registroLog('cancelarCargo', 'Cuenta por cobrar cancelada exitosamente', ['idcargo' => $_GET['id']]);

            $this->alerta(
                "Éxito",
                "La cuenta por cobrar ha sido cancelada correctamente.",
                "success",
                null,
                true,
                false,
            ...
            $database->pdo->rollBack();
            registroLog('cancelarCargo', 'Error al cancelar la cuenta por cobrar', ['error' => $e->getMessage()]);
            $this->alerta("Error", "No se pudo cancelar la cuenta por cobrar. Intenta de nuevo o contacta al administrador del sistema.", "error");
        }
...
$jaxon = jaxon();
$jaxon->register(Jaxon::CALLABLE_CLASS, ventasCxc::class);
